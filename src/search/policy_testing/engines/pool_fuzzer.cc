...
PoolFuzzerEngine::PoolFuzzerEngine(const plugins::Options &opts)
    : PolicyTestingBaseEngine(opts),
      ...
      bias_budget(static_cast<unsigned int>(std::max(opts.get<int>("bias_budget"), 0))),
...
void
PoolFuzzerEngine::add_options_to_feature(plugins::Feature &feature) {
    feature.add_option<int>("max_walk_length", "Maximal length of policy walks.", "5");
    ...
    feature.add_option<std::shared_ptr<PoolFilter>>("filter", "Pool filter (optional).", plugins::ArgumentInfo::NO_DEFAULT);
    feature.add_option<std::shared_ptr<Evaluator>>("eval", "Dead end heuristic (optional).", plugins::ArgumentInfo::NO_DEFAULT);
    ...
    feature.add_option<bool>("penalize_policy_fails",
...
PoolFuzzerEngine::random_walk() {
    const int ref_index = rng.random(pool.size());
    const int step_limit = rng.random(max_walk_length) + 1;
    ...
    for (; step_counter < step_limit; ++step_counter) {
        ...
        for (auto &applicable_op : applicable_ops) {
            ...
            if (!read_cached_bias) { // bias needs to be computed
                ...
                if (!task_properties::exists_applicable_op(task_proxy, succ)) {
                    if (cache_bias) {
                        bias_cache.emplace(succ.get_id(), FuzzingBias::NEGATIVE_INFINITY);
                    }
                    continue;
...
class PoolFuzzerFeature : public plugins::TypedFeature<SearchAlgorithm, PoolFuzzerEngine> {
public:
    PoolFuzzerFeature() : TypedFeature("pool_fuzzer") {
        PoolFuzzerEngine::add_options_to_feature(*this);
    }
};
static plugins::FeaturePlugin<PoolFuzzerFeature> _plugin;
} // namespace policy_testing
