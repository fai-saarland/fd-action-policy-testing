#include "slope_bias.h"
#include "../plugin.h"
#include "../../evaluation_context.h"

namespace policy_testing {
slopeBias::slopeBias(const options::Options &opts)
    ...
    if ((!h && !internalPlanCostEstimator) || (opts.contains("h") && opts.contains("ipo"))) {
        std::cerr << "Slope Bias needs either a heuristic or an internal planner oracle (and not both)\nh must be"
...
void
slopeBias::add_options_to_parser(options::OptionParser &parser) {
...
int
slopeBias::bias(const State &state, unsigned int budget) {
    ...
    for (int i = 0; i < path.size() - 1; ++i) {
        ...
        for (int j = i + 1; j < path.size(); ++j) {
            ...
            max_value = std::max<int>(max_value, new_value);
        }
    }
    return max_value;
}
...
bool
slopeBias::can_exclude_state(const State &s) {
    if (h) {
        EvaluationContext context(s);
        EvaluationResult result = h->compute_result(context);
...
static Plugin<FuzzingBias> _plugin_bias_slopeBias(
    "slope_bias",
    options::parse<PolicyBasedBias, slopeBias>);
} // namespace policy_testing
