...
IterativeImprovementOracle::IterativeImprovementOracle(const options::Options &opts)
    : NumericDominanceOracle(opts),
      upper_cost_bounds(Policy::UNSOLVED),
      max_state_comparisons(opts.get<unsigned int>("max_state_comparisons")),
      conduct_lookahead_search(opts.get<bool>("conduct_lookahead_search")),
      update_parents(opts.get<bool>("update_parents")),
      max_lookahead_state_comparisons(opts.get<unsigned int>("max_lookahead_state_comparisons")),
      ...
      max_lookahead_state_visits(opts.get<unsigned int>("max_lookahead_state_visits")),
...
void
IterativeImprovementOracle::add_options_to_parser(options::OptionParser &parser) {
    NumericDominanceOracle::add_options_to_parser(parser);
    parser.add_option<unsigned int>("max_state_comparisons", "Maximal number of states to compare bug candidates to",
                                    "1000000");
    parser.add_option<unsigned int>("max_lookahead_state_comparisons",
                                    "Maximal number of states to compare bug candidates to withing lookahead search",
                                    "1000000");
    parser.add_option<bool>("conduct_lookahead_search", "Enables lookahead search", "true");
    parser.add_option<bool>("update_parents", "Pass cost bounds to policy parent states", "true");
    parser.add_option<std::shared_ptr<Evaluator>>("lookahead_heuristic", "Heuristic to be used in lookahead search.",
                                                  options::OptionParser::NONE);
    parser.add_option<bool>("deferred_evaluation",
                            "Defer heuristic evaluation in lookahead_search.\n"
                            "Not implemented in qual_lookahead_search yet.",
                            "false");
    parser.add_option<bool>("domain_unit_cost_and_invertible",
                            "Performs optimizations assuming that the task is unit cost and the domain is invertible.",
                            "false");
    parser.add_option<unsigned int>("max_lookahead_state_visits",
                                    "Maximal number of states visited in lookahead search",
                                    "100");
    parser.add_enum_option<LookaheadComp>(
        "lookahead_comp", lookahead_comp_strings,
...
void IterativeImprovementOracle::removeState(const State &state, PolicyCost cost) {
    ...
    if (it == cost_set.end()) {
        std::cerr << "Trying to remove state with id " << state.get_id().get_value()
...
PolicyCost IterativeImprovementOracle::lookahead_search(Policy &policy, const State &s, unsigned int max_state_visits) {
    ...
    const successor_generator::SuccessorGenerator &generator = engine_->get_successor_generator();
    ...
    for (unsigned int i = 0; i < max_state_visits && !queue.empty();) {
        ...
        for (const auto &applicable_op : generator.generate_applicable_ops(current_state)) {
...
_plugin("iterative_improvement_oracle", options::parse<Oracle, IterativeImprovementOracle>);
} // namespace policy_testing
