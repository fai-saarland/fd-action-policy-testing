...
NumericDominanceOracle::add_options_to_feature(plugins::Feature &feature) {
    ...
    simulations::TauLabelManager<int>::add_options_to_feature(feature);
    feature.add_option<int>("truncate_value", "Assume -infinity if below minus this value", "1000");
    feature.add_option<int>("max_simulation_time",
                            "Maximum number of seconds spent in computing a single update of a simulation", "1800");
    feature.add_option<int>("min_simulation_time",
                            "Minimum number of seconds spent in computing a single update of a simulation", "1");
    feature.add_option<int>("max_total_time",
                            "Maximum number of seconds spent in computing all updates of a simulation", "7200");
    feature.add_option<int>("max_lts_size_to_compute_simulation",
                            "Avoid computing simulation on ltss that have more states than this number",
                            "1000000");
    feature.add_option<int>("num_labels_to_use_dominates_in",
                            "Use dominates_in for instances that have less than this amount of labels",
                            "0");
    feature.add_option<bool>("dump", "Dumps the relation that has been found", "false");
    feature.add_option<LocalBugTest>(
        "local_bug_test", "Apply local bug test not at all (NONE), only for the state it is called for (ONE) or for all states in "
    ...
    feature.add_option<bool>("write_sim_and_exit",
                             ...
                             "false");
    feature.add_option<bool>("read_simulation", "Read simulation from sim_file instead of computing it.", "false");
    feature.add_option<bool>("test_serialization", "Write simulation to disk, read it and make sure it coincides.",
                             "false");
}
