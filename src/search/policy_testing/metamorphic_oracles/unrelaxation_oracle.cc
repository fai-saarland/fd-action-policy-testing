...
UnrelaxationOracle::UnrelaxationOracle(const options::Options &opts)
    : NumericDominanceOracle(opts), operations_per_state(opts.get<unsigned int>("operations_per_state")),
...
void
UnrelaxationOracle::add_options_to_parser(options::OptionParser &parser) {
    NumericDominanceOracle::add_options_to_parser(parser);
    parser.add_option<unsigned int>("operations_per_state", "Number of unrelaxations to check in each state.", "4");
    parser.add_option<int>("max_evaluation_steps",
                           "Maximal number of steps in evaluation of policy in unrelaxed state.", "-1");
    parser.add_option<std::shared_ptr<Evaluator>>("dead_end_eval",
...
UnrelaxationOracle::unrelax(const State &s) const {
    ...
    for (int var = 0; var < num_variables; ++var) {
        ...
        for (int unrelaxed_value = 0; unrelaxed_value < domain_size; ++unrelaxed_value) {
            ...
            result.emplace_back(get_state_registry().insert_state(std::move(unrelaxed_state)), dominance_value);
        }
    }
...
_plugin("unrelaxation_oracle", options::parse<NumericDominanceOracle, UnrelaxationOracle>);
} // namespace policy_testing
