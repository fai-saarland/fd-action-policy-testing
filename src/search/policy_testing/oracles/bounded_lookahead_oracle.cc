...
BoundedLookaheadOracle::BoundedLookaheadOracle(const options::Options &opts)
    : Oracle(opts),
...
void
BoundedLookaheadOracle::add_options_to_parser(options::OptionParser &parser) {
    Oracle::add_options_to_parser(parser);
    parser.add_option<int>("depth", "", "2");
...
static Plugin<Oracle> _plugin(
    ...
    options::parse<Oracle, BoundedLookaheadOracle>);
} // namespace policy_testing
