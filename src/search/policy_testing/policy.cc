...
void Policy::add_options_to_feature(plugins::Feature &feature) {
    feature.add_option<int>("steps_limit",
                            "The maximal number of steps to execute the policy. 0 or negative value means no limit",
                            "0");
}
...
static class PolicyPlugin : public plugins::TypedCategoryPlugin<Policy> {
public:
    PolicyPlugin() : TypedCategoryPlugin("PolicyForTesting") {
        document_synopsis(
            "This page describes the different action policies."
            );
...
class CachedPolicyFeature : public plugins::TypedFeature<Policy, CachedPolicy> {
public:
    CachedPolicyFeature() : TypedFeature("cached_policy") {
        CachedPolicy::add_options_to_feature(*this);
    }
};

static plugins::FeaturePlugin<CachedPolicyFeature> _plugin;
} // namespace policy_testing
