...
LDSimulation::LDSimulation(bool unit_cost, const options::Options &opts, OperatorCost cost_type) :
...
#ifdef INCLUDE_SYM
template<typename T>
void LDSimulation::compute_satisficing_dominance_relation(int truncate_value,
                                                          int max_simulation_time,
                                                          int min_simulation_time, int max_total_time,
                                                          int max_lts_size_to_compute_simulation,
                                                          int num_labels_to_use_dominates_in,
    ...
    result = std::make_unique<T>(labels.get(),
                                 truncate_value, max_simulation_time, min_simulation_time, max_total_time,
                                 max_lts_size_to_compute_simulation, num_labels_to_use_dominates_in,
                                 tau_label_mgr);
...
void LDSimulation::compute_satisficing_dominance_relation(int truncate_value,
                                                          ...
                                                          bool dump,
                                                          std::shared_ptr<TauLabelManager<int>> tau_label_mgr,
                                                          std::unique_ptr<SatisficingDominanceRelation> &result) const;
#endif
...
// With the reduction methods we use here, this should just apply label reduction on all abstractions
    ...
    while (merge_strategy && !merge_strategy->done() && remaining_abstractions > 1 &&
        ...
        std::cout << std::endl << "Remaining: " << remaining_abstractions <<
            ...
            " memory: " << get_peak_memory_in_kb() << "/" << limit_memory_kb << " KB" << std::endl;
