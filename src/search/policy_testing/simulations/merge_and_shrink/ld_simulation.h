...
inline static const std::vector<std::string> LabelDominanceTypeValues{
    "NONE", "NOOP", "NORMAL", "ALTERNATIVE"
};

std::ostream &operator<<(std::ostream &os, const SimulationType &m);

inline static const std::vector<std::string> SimulationTypeValues{
    "NONE", "SIMPLE"
};
...
public:
    LDSimulation(bool unit_cost, const options::Options &opts, OperatorCost cost_type);
