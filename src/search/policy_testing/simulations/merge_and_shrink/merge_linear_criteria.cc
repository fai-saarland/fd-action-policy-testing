...
class MergeLinearCriteriaFeature : public plugins::TypedFeature<MergeStrategy, MergeLinearCriteria> {
public:
    MergeLinearCriteriaFeature() : TypedFeature("merge_linear_criteria") {
        add_option<MergeOrder>("var_order", "merge variable order for tie breaking", "RANDOM");
        add_list_option<std::shared_ptr<MergeCriterion>>("criteria", "list of criteria for the merge linear strategy");
    }
};
