...
void InfluenceGraph::randomize(std::vector<int> &ordering, std::vector<int> &new_order) {
    for (int i = 0; i < ordering.size(); i++) {
        int rnd_pos = order_rng(ordering.size() - i);
        int pos = -1;
        do {
            pos++;
            bool found;
...
long InfluenceGraph::optimize_variable_ordering_gamer(std::vector<int> &order,
    ...
    for (int counter = 0; counter < iterations; counter++) {
        //Swap variable
        int swapIndex1 = order_rng(order.size());
        int swapIndex2 = order_rng(order.size());
...
void InfluenceGraph::optimize_variable_ordering_gamer(std::vector<int> &order,
    ...
    for (int counter = 0; counter < iterations; counter++) {
        //Swap variable
        int partition = order_rng(partition_begin.size());
        if (partition_sizes[partition] <= 1)
            continue;
        int swapIndex1 = partition_begin[partition] + order_rng(partition_sizes[partition]);
        int swapIndex2 = partition_begin[partition] + order_rng(partition_sizes[partition]);
        if (swapIndex1 == swapIndex2)
            continue;
