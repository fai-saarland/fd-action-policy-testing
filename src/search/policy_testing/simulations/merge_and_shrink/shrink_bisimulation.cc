...
static std::shared_ptr<ShrinkStrategy> _parse_perfect(OptionParser & /*parser*/) {
    return ShrinkBisimulation::create_default(false, PLUS_INFINITY);
}
...
static std::shared_ptr<ShrinkStrategy> _parse(options::OptionParser &parser) {
    ...
    at_limit.emplace_back("RETURN");
    at_limit.emplace_back("USE_UP");
    parser.add_enum_option<ShrinkBisimulation::AtLimit>(
        "at_limit", at_limit,
...
static PluginTypePlugin<ShrinkBisimulation> _plugin_type_shrink_bisimulation("shrink_bisimulation", "");
static Plugin <ShrinkStrategy> _plugin("sim_shrink_bisimulation", _parse);
}
