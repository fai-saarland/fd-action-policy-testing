...
ShrinkStrategy::ShrinkStrategy(const plugins::Options &opts) : max_states(-1), max_states_before_merge(-1) {
    ...
    if (max_states_before_merge < 1) {
        std::cerr << "error: abstraction size before merge must be at least 1" << std::endl;
        utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
    }
...
void ShrinkStrategy::add_options_to_feature(plugins::Feature &feature) {
...
static class ShrinkStrategyPlugin : public plugins::TypedCategoryPlugin<ShrinkStrategy> {
public:
    ShrinkStrategyPlugin() : TypedCategoryPlugin("shrink_strategy") {
        document_synopsis(
            "This page describes the different shrink strategies."
            );
    }
}
_category_plugin;
