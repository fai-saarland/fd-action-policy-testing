    ...
    for (int i = 0; i < relation.size(); ++i) {
        for (int j = 0; j < relation.size(); ++j) {
            if (i == j && !simulates(i, j)) {
                std::cerr << "Assertion error: simulation relation is not reflexive" << std::endl;
                std::exit(0);
            }
            if (simulates(i, j)) {
...
const std::vector<BDD> &SimulationRelation::get_dominated_bdds() {
    if (dominated_bdds.empty())
        precompute_dominated_bdds();
    return dominated_bdds;
}

const std::vector<BDD> &SimulationRelation::get_dominating_bdds() {
    if (dominating_bdds.empty())
        precompute_dominating_bdds();
