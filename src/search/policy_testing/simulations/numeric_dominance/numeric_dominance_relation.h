    ...
    void compute_ld_simulation_template(std::vector<LTS *> &_ltss,
        ...
        do {
            do {
#ifdef EXTRA_VERBOSE
                if (dump) {
                    std::cout << "Current label dominance:" << std::endl;
                ...
                for (int i: order_by_size) {
                    // std::cout << "Updating LTS " << i << " of size " << _ltss[i]->size() << " states and "
...
template<>
std::unique_ptr<StrippedNumericDominanceRelation> NumericDominanceRelation<int>::strip(double computation_time) const {
...
template<>
std::unique_ptr<StrippedNumericDominanceRelation> NumericDominanceRelation<IntEpsilon>::strip(double) const {
