...
void NumericSimulationRelation<T>::precompute_bdds(bool dominating, bool quantified, bool use_ADD) {
    if (!quantified) {
        if (dominating) {
            std::cout << "Precomputing dominating_bdds ... " << std::endl;
            dominating_bdds.resize(abs->size(), vars->zeroBDD());
        } else {
            std::cout << "Precomputing dominated_bdds ... " << std::endl;
        ...
        std::cerr << "use_ADD not supported yet" << std::endl;
        exit(0);
        // if(dominating) {
        ...
        //      T value = may_simulate(i, j) ? q_simulates (i, j) : MINUS_INFINITY;
        ...
        if (dominating) {
            std::cout << "Precomputing dominating_bdd_maps ... " << std::endl;

            dominating_bdd_maps.resize(abs->size());
            may_dominating_bdds.resize(abs->size(), vars->zeroBDD());
        } else {
            std::cout << "Precomputing dominated_bdd_maps ... " << std::endl;
...
BDD NumericSimulationRelation<T>::getSimulatedBDD(const State &state) const {
    ...
    if (absstate == -1)
        ...
        return dominated_bdds[absstate];
}
...
BDD NumericSimulationRelation<T>::getSimulatingBDD(const State &state) const {
    ...
    if (absstate == -1)
        ...
        return dominating_bdds[absstate];
}


template<typename T>
const std::map <T, BDD> &NumericSimulationRelation<T>::getSimulatedBDDMap(const State &state) const {
...
template<typename T>
const std::map <T, BDD> &NumericSimulationRelation<T>::getSimulatingBDDMap(const State &state) const {
    assert(!dominated_bdd_maps.empty());
    int absstate = abs->get_abstract_state(state);
    assert(absstate == -1);
...
BDD NumericSimulationRelation<T>::getMaySimulatedBDD(const State &state) const {
    ...
    if (absstate == -1)
        ...
        return may_dominated_bdds[absstate];
}
...
BDD NumericSimulationRelation<T>::getMaySimulatingBDD(const State &state) const {
    ...
    if (absstate == -1)
        ...
        return may_dominating_bdds[absstate];
}
...
ADD NumericSimulationRelation<T>::getSimulatedADD(const State &state) const {
    ...
    int absstate = abs->get_abstract_state(state);
    if (absstate == -1)
        return vars->getADD(MINUS_INFINITY);
    else
        return dominated_adds[absstate];
}
...
ADD NumericSimulationRelation<T>::getSimulatingADD(const State &state) const {
    ...
    if (absstate == -1)
        ...
        return dominating_adds[absstate];
}
#endif
