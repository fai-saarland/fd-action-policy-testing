...
public:
    ...
    [[nodiscard]] BDD getSimulatedBDD(const State &state) const;
    [[nodiscard]] BDD getSimulatingBDD(const State &state) const;
    ...
    [[nodiscard]] BDD getMaySimulatedBDD(const State &state) const;
    [[nodiscard]] BDD getMaySimulatingBDD(const State &state) const;
    [[nodiscard]] ADD getSimulatedADD(const State &state) const;
    [[nodiscard]] ADD getSimulatingADD(const State &state) const;
#endif
...
template<>
std::unique_ptr<StrippedNumericSimulationRelation> NumericSimulationRelation<int>::strip() const {
    return std::make_unique<StrippedNumericSimulationRelation>(abs->strip(), relation);
}

template<>
std::unique_ptr<StrippedNumericSimulationRelation> NumericSimulationRelation<IntEpsilon>::strip() const {
    throw std::logic_error("Stripping NumericSimulationRelation<IntEpsilon> is not supported.");
}

template<>
int NumericSimulationRelation<int>::get_min_finite_entry() const {
    int result = 0;
    for (unsigned int i = 0; i < relation.size(); ++i) {
        for (unsigned int j = 0; j < relation.size(); ++j) {
            if (i == j)
...
template<>
int NumericSimulationRelation<IntEpsilon>::get_min_finite_entry() const {
