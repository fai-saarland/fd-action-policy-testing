...
//Auxiliary file to declare all enumerate values Each enumerate has:
//its definition, ostream << operator and an array with representative
//strings. The order of the values in the enumeration must correspond
...
enum class AbsTRsStrategy {
    TR_SHRINK, IND_TR_SHRINK, REBUILD_TRS,
    SHRINK_AFTER_IMG
};
...
enum UCTRewardType {
    STATES, NODES, STATES_TIME, NODES_TIME, STATES_NODES, NONE, RAND
};
